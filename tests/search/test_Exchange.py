...
def test_relocate_after_depot_should_work(ok_small):
    ...
    assert_(op.evaluate(nodes[-1], route2[0], cost_evaluator) < Cost(0))
...
def test_relocate_fixed_vehicle_cost(ok_small, op, base_cost, fixed_cost):
    ...
    assert_equal(
        op.evaluate(route1[1], route2[0], cost_eval),
        Cost(base_cost + fixed_cost),
    )
...
def test_exchange_with_duration_constraint(ok_small, op, max_dur, cost):
    ...
    cost_eval = CostEvaluator([1], 1, 0)
    assert_equal(op.evaluate(route1[1], route2[1], cost_eval), Cost(cost))
...
def test_within_route_simultaneous_pickup_and_delivery(operator):
    ...
    op = operator(data)
    cost_eval = CostEvaluator([1], 1, 0)
    assert_equal(op.evaluate(route[1], route[3], cost_eval), Cost(-5))
...
def test_relocate_max_distance(ok_small, max_distance: int, expected: int):
    ...
    assert_equal(op.evaluate(route1[2], route2[0], cost_eval), Cost(expected))
...
def test_swap_max_distance(ok_small, max_distance: int, expected: int):
    ...
    assert_equal(op.evaluate(route1[2], route2[1], cost_eval), Cost(expected))
...
def test_swap_with_different_profiles(ok_small_two_profiles):
    ...
    assert_equal(op.evaluate(route1[1], route2[1], cost_eval), Cost(delta))
...
def test_swap_does_not_swap_depots(ok_small_multiple_trips):
    ...
    assert_equal(op.evaluate(route[2], route[4], cost_eval), Cost(0))
...
def test_bug_evaluating_move_with_initial_load():
    ...
    assert_equal(op.evaluate(route1[1], route2[1], cost_eval), Cost(0))
...
def test_bug_release_time_shift_time_windows():
    ...
    assert_equal(op.evaluate(route1[1], route2[0], cost_eval), Cost(0))
