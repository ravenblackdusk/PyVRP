...
def test_inserts_depot_single_route(ok_small_multiple_trips):
    ...
    assert_equal(op.evaluate(route[2], route[3], cost_eval), Cost(-907))
...
def test_inserts_depot_across_routes(ok_small_multiple_trips):
    ...
    # delta is thus -3_052.
    assert_equal(op.evaluate(route1[1], route2[1], cost_eval), Cost(-3_052))
...
def test_reload_depot_before_or_after_relocate(
    ...
    assert_equal(op.evaluate(route[1], route[2], cost_eval), Cost(exp_delta_cost))
...
def test_inserts_best_reload_depot():
    ...
    assert_equal(op.evaluate(route[1], route[2], cost_eval), Cost(-2_500))
...
def test_fixed_vehicle_cost():
    ...
    assert_equal(op.evaluate(route1[1], route2[1], cost_eval), Cost(-2_000))
...
def test_does_not_evaluate_if_already_max_trips(ok_small_multiple_trips):
    ...
    assert_equal(op.evaluate(route[3], route[4], cost_eval), Cost(0))
...
def test_bug_release_times(mtvrptw_release_times):
    ...
    assert_equal(delta_cost, Cost(delta_dist + delta_time_warp))
...
def test_can_insert_reload_after_start_depot():
    ...
    assert_equal(op.evaluate(route[2], route[0], cost_eval), Cost(-2))
...
def test_can_insert_reload_before_end_depot():
    ...
    assert_equal(op.evaluate(route[1], route[2], cost_eval), Cost(-10))
