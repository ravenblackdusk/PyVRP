...
from pyvrp import Cost, PenaltyManager, PenaltyParams, Solution, VehicleType
...
def test_repair_booster():
    ...
    assert_equal(cost_evaluator.tw_penalty(1), Cost(1))
    assert_equal(cost_evaluator.load_penalty(2, 1, 0), Cost(1))  # 1 unit above cap
    ...
    assert_equal(booster.tw_penalty(1), Cost(5))
    assert_equal(booster.tw_penalty(2), Cost(10))

    assert_equal(booster.load_penalty(2, 1, 0), Cost(5))  # 1 unit above capacity
    assert_equal(booster.load_penalty(3, 1, 0), Cost(10))  # 2 units above capacity
...
def test_load_penalty_update_increase(ok_small):
    ...
    assert_equal(pm.cost_evaluator().load_penalty(2, 1, 0), Cost(1))
    ...
    assert_equal(pm.cost_evaluator().load_penalty(2, 1, 0), Cost(100))
    for sol in [infeas] * num_registrations:
        pm.register(sol)
    assert_equal(pm.cost_evaluator().load_penalty(2, 1, 0), Cost(110))
...
def test_load_penalty_update_decrease(ok_small):
    ...
    # Within bandwidth, so penalty should not change.
    assert_equal(pm.cost_evaluator().load_penalty(2, 1, 0), Cost(4))
    for sol in [feas, infeas, feas, infeas]:
        pm.register(sol)
    assert_equal(pm.cost_evaluator().load_penalty(2, 1, 0), Cost(4))
    ...
    for sol in [feas] * num_registrations:
        pm.register(sol)
    assert_equal(pm.cost_evaluator().load_penalty(2, 1, 0), Cost(3))
    ...
    for sol in [feas] * num_registrations:
        pm.register(sol)
    assert_equal(pm.cost_evaluator().load_penalty(2, 1, 0), Cost(90))

    # Test that the penalty cannot decrease beyond min_penalty.
    params = PenaltyParams(1, num_registrations, 1.1, 0.9, 0.5)
    pm = PenaltyManager(([0.1], 1, 1), params)

    assert_equal(pm.cost_evaluator().load_penalty(11, 1, 0), Cost(1))
    for sol in [feas] * num_registrations:
        pm.register(sol)
    assert_equal(pm.cost_evaluator().load_penalty(11, 1, 0), Cost(1))
...
def test_time_warp_penalty_update_increase(ok_small):
    ...
    # Within bandwidth, so penalty should not change.
    assert_equal(pm.cost_evaluator().tw_penalty(1), Cost(1))
    for sol in [feas, infeas, feas, infeas]:
        pm.register(sol)
    assert_equal(pm.cost_evaluator().tw_penalty(1), Cost(1))
    ...
    assert_equal(pm.cost_evaluator().tw_penalty(1), Cost(100))
    for sol in [infeas] * num_registrations:
        pm.register(sol)
    assert_equal(pm.cost_evaluator().tw_penalty(1), Cost(110))
...
def test_time_warp_penalty_update_decrease(ok_small):
    ...
    # Within bandwidth, so penalty should not change.
    assert_equal(pm.cost_evaluator().tw_penalty(1), Cost(4))
    for sol in [feas, infeas, feas, infeas]:
        pm.register(sol)
    assert_equal(pm.cost_evaluator().tw_penalty(1), Cost(4))
    ...
    for sol in [feas] * num_registrations:
        pm.register(sol)
    assert_equal(pm.cost_evaluator().tw_penalty(1), Cost(3))
    ...
    for sol in [feas] * num_registrations:
        pm.register(sol)
    assert_equal(pm.cost_evaluator().tw_penalty(1), Cost(90))

    # Test that the penalty cannot decrease beyond min_penalty.
    params = PenaltyParams(1, num_registrations, 1.1, 0.9, 0.5)
    pm = PenaltyManager(([1], 0.1, 1), params)

    assert_equal(pm.cost_evaluator().tw_penalty(10), Cost(1))
    for sol in [feas] * num_registrations:
        pm.register(sol)
    assert_equal(pm.cost_evaluator().tw_penalty(10), Cost(1))
...
def test_does_not_update_penalties_before_sufficient_registrations(ok_small):
    ...
    assert_equal(pm.cost_evaluator().tw_penalty(1), Cost(4))
    assert_equal(pm.cost_evaluator().load_penalty(2, 1, 0), Cost(4))
    assert_equal(pm.cost_evaluator().dist_penalty(2, 1), Cost(4))
    ...
    for sol in [feas, infeas, feas]:
        ...
        assert_equal(pm.cost_evaluator().tw_penalty(1), Cost(4))
        assert_equal(pm.cost_evaluator().load_penalty(2, 1, 0), Cost(4))
        assert_equal(pm.cost_evaluator().dist_penalty(1, 0), Cost(4))
    ...
    assert_equal(pm.cost_evaluator().load_penalty(2, 1, 0), Cost(3))
    assert_equal(pm.cost_evaluator().tw_penalty(1), Cost(3))
    assert_equal(pm.cost_evaluator().dist_penalty(1, 0), Cost(3))
...
def test_max_min_penalty(ok_small):
    ...
    assert_equal(pm.cost_evaluator().tw_penalty(1), Cost(int(params.max_penalty)))
    ...
    pm.register(feas)
    assert_allclose(
        int(pm.cost_evaluator().tw_penalty(10)), 10 * params.min_penalty
    )
...
def test_init_from_load_penalty_value(ok_small):
    ...
    assert_equal(cost_eval.load_penalty(1, 0, 0), Cost(int(avg_cost / avg_load)))
...
def test_init_from_tw_penalty_value(ok_small):
    ...
    assert_equal(cost_eval.tw_penalty(1), Cost(round(avg_distance / avg_duration)))
...
def test_init_from_different_unit_costs(ok_small):
    ...
    assert_equal(cost_eval.tw_penalty(1), Cost(round(avg_cost / avg_duration)))
    assert_equal(cost_eval.dist_penalty(1, 0), Cost(round(avg_cost / avg_distance)))
...
def test_init_clips_penalties():
    ...
    cost_eval = pm.cost_evaluator()
    assert_allclose(  # MIN
        int(cost_eval.load_penalty(10, 0, 0)),
        10 * params.min_penalty,
    )
    assert_equal(cost_eval.tw_penalty(1), Cost(int(params.max_penalty)))  # MAX
    assert_equal(cost_eval.dist_penalty(1, 0), Cost(2))  # already OK, so unchanged
